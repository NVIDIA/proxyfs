...
func benchmarkRpcLeaseCustomServer(useTLS bool, doneWG *sync.WaitGroup) {
	...
	var (
		...
		myUniqueClient = "CustomServer"
	)

	jserver = NewServer()
	bucketstats.Register("benchCustomServer", myUniqueClient, &myClient1)
	...
	for {
		...
		if nil != err {
			...
			doneWG.Done()
			fmt.Printf("BUCKETSTATS: %v\n", bucketstats.SprintStats(bucketstats.StatFormatParsable1, "benchCustomServer", "*"))
			bucketstats.UnRegister("benchCustomServer", myUniqueClient)
