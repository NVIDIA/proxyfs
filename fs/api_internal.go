...
func (vS *volumeStruct) inFlightFileInodeDataFlusher(inodeNumber inode.InodeNumber) {
	...
	inodeLock, err = inode.InitInodeLock(vS.volumeName, inodeNumber, nil)
	if nil != err {
		logger.PanicfWithError(err, "inode.InitInodeLock() for volume '%s' inode %d failed", vS.volumeName, inodeNumber)
	}
...
func (mS *mountStruct) Create(userID inode.InodeUserID, groupID inode.InodeGroupID, otherGroupIDs []inode.InodeGroupID, dirInodeNumber inode.InodeNumber, basename string, filePerm inode.InodeMode) (fileInodeNumber inode.InodeNumber, err error) {
	...
	dirInodeLock, err := inode.InitInodeLock(mS.VolumeName(), dirInodeNumber, nil)
	if err != nil {
		return 0, err
	}
...
func (mS *mountStruct) getstatHelper(inodeNumber inode.InodeNumber, callerID dlm.CallerID) (stat Stat, err error) {
	lockID, err := inode.MakeLockID(mS.VolumeName(), inodeNumber)
	if err != nil {
		return
	}
...
func (mS *mountStruct) Link(userID inode.InodeUserID, groupID inode.InodeGroupID, otherGroupIDs []inode.InodeGroupID, dirInodeNumber inode.InodeNumber, basename string, targetInodeNumber inode.InodeNumber) (err error) {
	...
	dirInodeLock, err := inode.InitInodeLock(mS.VolumeName(), dirInodeNumber, callerID)
	if err != nil {
		return
	}

	targetInodeLock, err := inode.InitInodeLock(mS.VolumeName(), targetInodeNumber, callerID)
	if err != nil {
		return
	}
...
func (mS *mountStruct) LookupPath(userID inode.InodeUserID, groupID inode.InodeGroupID, otherGroupIDs []inode.InodeGroupID, fullpath string) (inodeNumber inode.InodeNumber, err error) {
	...
	for _, segment := range pathSegments {
		cursorInodeLock, err1 := inode.InitInodeLock(mS.VolumeName(), cursorInodeNumber, nil)
		if err = err1; err != nil {
			return
		}
...
func (mS *mountStruct) MiddlewareCoalesce(destPath string, elementPaths []string) (ino uint64, numWrites uint64, modificationTime uint64, err error) {
	...
	rootDirInodeLock, err := inode.GetWriteLock(mS.VolumeName(), inode.RootDirInodeNumber, callerID)
	if err != nil {
		return
	}
	...
	for len(destDirPathComponents) > 0 {
		...
		nextCursorInodeNumber, nextCursorInodeType, nextCursorInodeLock, err1 := mS.resolvePath(pathComponent, callerID, cursorInodeNumber, inode.EnsureWriteLock)
		if err1 != nil {
			err = err1
			return
		}
	...
	for len(destDirPathComponents) > 0 {
		...
		cursorInodeLock, err1 = inode.EnsureWriteLock(mS.VolumeName(), newDirInodeNumber, callerID)
		if err1 != nil {
			err = err1
			return
		}
	...
	for _, entry := range elementDirAndFileNames {
		...
		fileInodeLock, err1 := inode.InitInodeLock(mS.VolumeName(), fileInodeNumber, callerID)
		if err1 != nil {
			err = err1
			return
		}
...
func (mS *mountStruct) MiddlewareDelete(parentDir string, baseName string) (err error) {
	...
	baseInodeLock, err := inode.GetWriteLock(mS.VolumeName(), baseNameInodeNumber, parentDirLock.GetCallerID())
	if nil != err {
		return
	}
...
func putObjectHelper(mS *mountStruct, vContainerName string, vObjectPath string, makeInodeFunc func() (inode.InodeNumber, error)) (mtime uint64, ctime uint64, fileInodeNumber inode.InodeNumber, numWrites uint64, err error) {
	...
	callerID := dlm.GenerateCallerID()
	dirInodeLock, err := inode.GetWriteLock(mS.VolumeName(), dirInodeNumber, callerID)
	if err != nil {
		return
	}
	defer func() {
	...
	for len(dirs) > 0 {
		...
		if err1 != nil && blunder.Errno(err1) == int(blunder.NotFoundError) {
			...
			dirs = dirs[0 : len(dirs)-1]
			dirEntInodeLock, err1 = inode.GetWriteLock(mS.VolumeName(), dirEntInodeNumber, callerID)
			if err1 != nil {
				err = err1
				return
			}
			...
			if dirEntInodeType == inode.FileType {
				...
				if strings.HasPrefix(target, "/") {
					...
					dirInodeLock, err1 = inode.GetWriteLock(mS.VolumeName(), inode.RootDirInodeNumber, nil)
					if err1 != nil {
						err = err1
						return
					}
	...
	if 0 == len(dirs) {
		...
		if err1 != nil && blunder.Errno(err1) == int(blunder.NotFoundError) {
			...
			obstacleInodeLock, err1 := inode.GetWriteLock(mS.VolumeName(), obstacleInodeNumber, callerID)
			if err1 != nil {
				err = err1
				return
			}
...
func (mS *mountStruct) MiddlewarePutContainer(containerName string, oldMetadata []byte, newMetadata []byte) (err error) {
	...
	// much else we can do.
	rootInodeLock, err := inode.GetWriteLock(mS.VolumeName(), inode.RootDirInodeNumber, nil)
	if nil != err {
		return
	}
	...
	if err != nil && blunder.IsNot(err, blunder.NotFoundError) {
		...
		newDirInodeLock, err = inode.GetWriteLock(mS.VolumeName(), newDirInodeNumber, nil)
	...
	containerInodeLock, err = inode.GetWriteLock(mS.VolumeName(), containerInodeNumber, nil)
	if err != nil {
		return
	}
...
func (mS *mountStruct) Rename(userID inode.InodeUserID, groupID inode.InodeGroupID, otherGroupIDs []inode.InodeGroupID, srcDirInodeNumber inode.InodeNumber, srcBasename string, dstDirInodeNumber inode.InodeNumber, dstBasename string) (err error) {
	...
	srcDirLock, err := inode.InitInodeLock(mS.VolumeName(), srcDirInodeNumber, callerID)
	if err != nil {
		return
	}

	// Allocate the dest dir lock
	dstDirLock, err := inode.InitInodeLock(mS.VolumeName(), dstDirInodeNumber, callerID)
	if err != nil {
		return err
	}
...
func (mS *mountStruct) ReaddirPlus(userID inode.InodeUserID, groupID inode.InodeGroupID, otherGroupIDs []inode.InodeGroupID, inodeNumber inode.InodeNumber, prevBasenameReturned string, maxEntries uint64, maxBufSize uint64) (dirEntries []inode.DirEntry, statEntries []Stat, numEntries uint64, areMoreEntries bool, err error) {
	...
	for i := range dirEntries {
		entryInodeLock, err1 := inode.InitInodeLock(mS.VolumeName(), dirEntries[i].InodeNumber, nil)
		if err = err1; err != nil {
			return
		}
...
func (mS *mountStruct) Rmdir(userID inode.InodeUserID, groupID inode.InodeGroupID, otherGroupIDs []inode.InodeGroupID, inodeNumber inode.InodeNumber, basename string) (err error) {
	...
	callerID := dlm.GenerateCallerID()
	inodeLock, err := inode.InitInodeLock(mS.VolumeName(), inodeNumber, callerID)
	if err != nil {
		return
	}
	err = inodeLock.WriteLock()
	...
	basenameInodeLock, err := inode.InitInodeLock(mS.VolumeName(), basenameInodeNumber, callerID)
	if err != nil {
		return
	}
...
func (mS *mountStruct) resolvePathForRead(fullpath string, callerID dlm.CallerID) (inodeNumber inode.InodeNumber, inodeType inode.InodeType, inodeLock *dlm.RWLockStruct, err error) {
	return mS.resolvePath(fullpath, callerID, inode.RootDirInodeNumber, inode.EnsureReadLock)
}
...
func (mS *mountStruct) resolvePathForWrite(fullpath string, callerID dlm.CallerID) (inodeNumber inode.InodeNumber, inodeType inode.InodeType, inodeLock *dlm.RWLockStruct, err error) {
	return mS.resolvePath(fullpath, callerID, inode.RootDirInodeNumber, inode.EnsureWriteLock)
}

func (mS *mountStruct) resolvePath(fullpath string, callerID dlm.CallerID, startingInode inode.InodeNumber, getLock func(string, inode.InodeNumber, dlm.CallerID) (*dlm.RWLockStruct, error)) (inodeNumber inode.InodeNumber, inodeType inode.InodeType, inodeLock *dlm.RWLockStruct, err error) {
	...
	dirInodeLock, err := getLock(mS.VolumeName(), dirInodeNumber, callerID)
	...
	for len(pathSegments) > 0 {
		...
		cursorInodeLock, err = getLock(mS.VolumeName(), cursorInodeNumber, callerID)
		if err != nil {
			return
		}
		...
		if cursorInodeType == inode.SymlinkType {
			...
			if strings.HasPrefix(target, "/") {
				...
				dirInodeNumber = inode.RootDirInodeNumber
				dirInodeLock, err = getLock(mS.VolumeName(), inode.RootDirInodeNumber, callerID)
				if err != nil {
					return
				}
			}
...
func (mS *mountStruct) readdirHelper(inodeNumber inode.InodeNumber, prevBasenameReturned string, maxEntries uint64, maxBufSize uint64, callerID dlm.CallerID) (entries []inode.DirEntry, numEntries uint64, areMoreEntries bool, err error) {
	...
	for i := range entries {
		if inodeNumber == entries[i].InodeNumber {
			...
			entryInodeLock, err1 := inode.InitInodeLock(mS.VolumeName(), entries[i].InodeNumber, callerID)
			if err = err1; err != nil {
				return
			}
			err = entryInodeLock.ReadLock()
